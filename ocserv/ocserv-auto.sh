...
if [ x$1 == x ]
then
   printf "No Server IP, Sample: ./ocserv-auto.sh 120.12.12.1\n" #...没有参数
   exit 1;
else
    serverIP=$1
fi
...
_EOF_
    ...
    sed -i "s/listen-host = serverIP/listen-host = ${serverIP}/g" "${confdir}/ocserv.conf"
...
#oute = 192.42.249.0/255.255.255.0
...
_EOF_
        ...
        certtool --to-p12 --load-privkey user-key.pem --pkcs-cipher \
        3des-pkcs12 --load-certificate user-cert.pem --outfile user.p12 \
        --outder
    fi
